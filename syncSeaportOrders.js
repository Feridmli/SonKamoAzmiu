...
async function postOrderEvent(payload) {
  try {
    const res = await fetch(`${BACKEND_URL}/api/order`, {  // <-- Düzəliş
      method: "POST",
...
async function main() {
  ...
  await queryInChunks(async (start, end) => {
    ...
    for (const ev of events) {
      ...
      await postOrderEvent(payload) && console.log(`✅ Primary Fulfilled: ${args.orderHash}`);
    }
  ...
  await queryInChunks(async (start, end) => {
    ...
    for (const ev of events) {
      ...
      await postOrderEvent(payload) && console.log(`✅ Alt Fulfilled: ${args.orderHash}`);
    }
  }, FROM_BLOCK, latestBlock);
  ...
  await queryInChunks(async (start, end) => {
    ...
    for (const ev of events) {
      ...
      await postOrderEvent(payload) && console.log(`🗑 Cancelled: ${args.orderHash}`);
    }
  }, FROM_BLOCK, latestBlock);
